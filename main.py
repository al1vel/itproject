...
async def booking_recommendation(request: Request, login: str, date: str = None):
    ...
    cursor.execute("SELECT room_name FROM Rooms_Information")
    room_data = cursor.fetchall()
    return templates.TemplateResponse("main_page.html", {"request": request, "login": login, "recommended_rooms": recommended_rooms, "location_options": location_options, "room_data": room_data})
